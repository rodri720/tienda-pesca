...
class Database {
        ...
        return new Promise((resolve, reject) => {
            ...
            `, (err) => {
                ...
                `, (err) => {
                    ...
                    // Tabla gastos: usar IF NOT EXISTS y luego agregar columnas faltantes
                    this.db.run(`
            ...
            try {
                ...
                this.db.run(
                    `INSERT INTO productos 
                     (id, sku, nombre, descripcion, categoria, marca, precio, costo, stock, stock_minimo, stock_apertura, proveedor)
                     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
                    [
                        id,
                        producto.sku,
            ...
            this.db.all(
                ...
                (err, rows) => {
                    if (err) {
                        ...
                        // Parsear JSON de ventas_por_metodo
                        rows.forEach(row => {
                            try {
    ...
    /**
     * Guarda o reemplaza el resumen de caja de un día específico.
     * @param {Object} resumen - Objeto con fecha, saldo_inicial, total_ingresos, total_egresos, saldo_final, movimientos (array)
     * @returns {Promise<Object>} El resumen guardado.
     */
    async saveCajaDiaria(resumen) {
        return new Promise((resolve, reject) => {
            const id = uuidv4();
            const fecha = resumen.fecha; // formato YYYY-MM-DD
            const movimientosJSON = JSON.stringify(resumen.movimientos || []);
            
            // Usamos INSERT OR REPLACE para que si ya existe la fecha, se actualice
            this.db.run(
                ...
                function(err) {
                    if (err) {
                        ...
                        // Devolver el objeto con id asignado
                        resolve({ id, ...resumen });
                    }
                }
            );
     ...
     * @returns {Promise<Object|null>} El resumen o null si no existe.
     */
    ...
    /**
     * Obtiene los últimos N resúmenes de caja ordenados por fecha descendente.
