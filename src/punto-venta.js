...
// NUEVO: PAGOS MIXTOS - permite dividir el pago entre múltiples métodos de pago (botón adicional)
// NUEVO: Ventas pendientes (crédito Banco Nación) - botón en barra de caja y en saldo de caja
// MODIFICADO: Al cobrar pendiente se registra como ingreso en EFECTIVO
// MODIFICADO: Botones de filtro por método de pago en el modal de SALDO DE CAJA (ahora buscan en todas las ventas históricas)
...
class PuntoVentaApp {
    constructor() {
        ...
        this.bancoNacionTipo = 'debito'; // 'debito' o 'credito'
    ...
    init() {
        ...
        // 👇 NUEVO: Agregar botón de ventas pendientes
        this.agregarBotonPendientes();
    ...
    cargarDatos() {
        try {
            ...
            // 👇 NUEVO: inicializar ventas pendientes
            if (!localStorage.getItem('ventasPendientes')) {
                localStorage.setItem('ventasPendientes', JSON.stringify([]));
            }
    ...
    // ========== NUEVO: Funciones para ventas pendientes ==========

    agregarBotonPendientes() {
        const btnSaldo = document.getElementById('btn-saldo-caja');
        if (btnSaldo && btnSaldo.parentNode) {
            if (document.getElementById('btn-ventas-pendientes')) return;
            const btnPendientes = document.createElement('button');
            btnPendientes.id = 'btn-ventas-pendientes';
            btnPendientes.className = 'btn btn-warning';
            btnPendientes.innerHTML = '<i class="fas fa-clock"></i> Pendientes';
            btnPendientes.style.marginLeft = '10px';
            btnPendientes.addEventListener('click', (e) => {
                e.preventDefault();
                this.mostrarVentasPendientes();
            });
            btnSaldo.parentNode.appendChild(btnPendientes);
        }
    }

    cargarVentasPendientes() {
        return JSON.parse(localStorage.getItem('ventasPendientes')) || [];
    }

    guardarVentasPendientes(pendientes) {
        localStorage.setItem('ventasPendientes', JSON.stringify(pendientes));
        if (window.api && window.api.savePendientes) {
            try {
                window.api.savePendientes(pendientes);
            } catch (e) {
                console.warn('No se pudo guardar pendientes en BD:', e);
            }
        }
    }

    async sincronizarPendientesConBD() {
        if (window.api && window.api.getPendientes) {
            try {
                const pendientesBD = await window.api.getPendientes();
                if (pendientesBD && pendientesBD.length > 0) {
                    localStorage.setItem('ventasPendientes', JSON.stringify(pendientesBD));
                    return pendientesBD;
                }
            } catch (e) {
                console.warn('Error al sincronizar pendientes con BD:', e);
            }
        }
        return this.cargarVentasPendientes();
    }

    async guardarPendienteEnBD(venta) {
        if (window.api && window.api.createVenta) {
            try {
                await window.api.createVenta({ ...venta, estado: 'pendiente' });
            } catch (e) {
                console.warn('Error al guardar pendiente en BD:', e);
            }
        }
    }

    async actualizarPendienteEnBD(id, datos) {
        if (window.api && window.api.updateVenta) {
            try {
                await window.api.updateVenta(id, datos);
            } catch (e) {
                console.warn('Error al actualizar pendiente en BD:', e);
            }
        }
    }

    async mostrarVentasPendientes() {
        const pendientes = await this.sincronizarPendientesConBD();
        if (pendientes.length === 0) {
            alert('No hay ventas pendientes.');
            return;
        }

        let modal = document.getElementById('modal-pendientes');
        if (!modal) {
            modal = document.createElement('div');
            modal.id = 'modal-pendientes';
        ...
        let html = `
            <div class="modal-content" style="max-width: 800px;">
                <div class="modal-header">
                    <h3><i class="fas fa-clock"></i> Ventas Pendientes (${pendientes.length})</h3>
                    <button class="btn-cerrar-modal" data-modal="modal-pendientes" style="background: none; border: none; font-size: 1.5rem; cursor: pointer;">
                        <i class="fas fa-times"></i>
                    </button>
                </div>
                <div style="padding: 20px; max-height: 500px; overflow-y: auto;">
                    <button class="btn btn-success" onclick="app.cobrarTodasPendientes()" style="margin-bottom: 15px;">
                        <i class="fas fa-check-double"></i> Cobrar todas
                    </button>
        `;
            ...
            html += `
                <div style="border: 1px solid #e2e8f0; border-radius: 8px; padding: 15px; margin-bottom: 10px;">
                    <div style="display: flex; justify-content: space-between;">
                        <div>
                            <strong>Venta #${venta.id}</strong> - ${fecha}
                            <div>Total: $${venta.total.toFixed(2)} (${venta.metodo_pago})</div>
                            <div>Productos: ${venta.productos.map(p => `${p.cantidad} x ${p.nombre}`).join(', ')}</div>
                        </div>
                        <div>
                            <button class="btn btn-primary btn-sm" onclick="app.cobrarPendiente(${index})">
                                <i class="fas fa-hand-holding-usd"></i> Cobrar
                            </button>
                            <button class="btn btn-danger btn-sm" onclick="app.eliminarPendiente(${index})">
        ...
        html += `
                ...
                <div class="form-actions" style="padding: 20px;">
                    <button class="btn btn-secondary btn-cerrar-modal" data-modal="modal-pendientes">Cerrar</button>
                </div>
            </div>
        `;

        modal.innerHTML = html;
        this.abrirModal('modal-pendientes');
    }

    // MODIFICADO: cobrarPendiente ahora registra el ingreso en EFECTIVO
    async cobrarPendiente(index) {
        const pendientes = this.cargarVentasPendientes();
        if (index < 0 || index >= pendientes.length) return;
        const venta = pendientes[index];
        if (!confirm(`¿Cobrar la venta #${venta.id} por $${venta.total.toFixed(2)}?`)) return;

        const neto = venta.total_neto || (venta.total - (venta.comision_monto || 0));
        // 👇 Se registra como ingreso en EFECTIVO
        this.agregarIngreso(neto, `Cobro pendiente #${venta.id}`, 'efectivo', venta.id.toString());

        pendientes.splice(index, 1);
        this.guardarVentasPendientes(pendientes);
        if (window.api && window.api.updateVenta) {
            try {
                await window.api.updateVenta(venta.id, { estado: 'cobrada' });
            } catch (e) {
                console.warn('Error al actualizar estado en BD:', e);
            }
        }

        this.mostrarNotificacion(`✅ Venta #${venta.id} cobrada (efectivo). Neto: $${neto.toFixed(2)}`);
        this.cerrarModal('modal-pendientes');
        this.mostrarVentasPendientes();
    }

    // MODIFICADO: cobrarTodasPendientes también en EFECTIVO
    async cobrarTodasPendientes() {
        const pendientes = this.cargarVentasPendientes();
        if (pendientes.length === 0) return;
        if (!confirm(`¿Cobrar todas las ${pendientes.length} ventas pendientes por un total de $${pendientes.reduce((s, v) => s + v.total, 0).toFixed(2)}?`)) return;
        ...
        this.mostrarNotificacion(`✅ ${pendientes.length} ventas pendientes cobradas (efectivo).`);
        this.cerrarModal('modal-pendientes');
        this.mostrarVentasPendientes();
    }
    ...
    eliminarPendiente(index) {
        const pendientes = this.cargarVentasPendientes();
        if (index < 0 || index >= pendientes.length) return;
        if (!confirm(`¿Eliminar la venta pendiente #${pendientes[index].id}? (No afecta stock ni caja)`)) return;
        pendientes.splice(index, 1);
        this.guardarVentasPendientes(pendientes);
        this.mostrarVentasPendientes();
    }

    // ========== NUEVO: Mostrar ventas por método (AHORA BUSCA EN TODAS LAS VENTAS HISTÓRICAS) ==========
    mostrarVentasPorMetodo(metodo) {
        // Combinar ventas del día y ventas históricas
        ...
        const todasLasVentas = [...ventasDelDia, ...ventasHistoricas];
        
        // Filtrar por método de pago (manejar pagos mixtos y simples)
        const ventasFiltradas = todasLasVentas.filter(v => {
            if (v.pagos && v.pagos.length > 0) {
                // Si es pago mixto, buscar si algún pago coincide con el método
                return v.pagos.some(p => p.metodo === metodo);
            } else {
        ...
        if (ventasFiltradas.length === 0) {
            alert(`No hay ventas registradas con el método ${metodo.toUpperCase()} en todo el historial.`);
            return;
        }

        let modal = document.getElementById('modal-ventas-metodo');
        if (!modal) {
            modal = document.createElement('div');
            modal.id = 'modal-ventas-metodo';
            modal.className = 'modal';
            document.body.appendChild(modal);
        }

        let total = ventasFiltradas.reduce((sum, v) => sum + (v.total_final || v.total), 0);

        let html = `
            <div class="modal-content" style="max-width: 700px;">
                <div class="modal-header">
                    <h3><i class="fas fa-list"></i> Ventas en ${metodo.toUpperCase()} (${ventasFiltradas.length} registros)</h3>
                    <button class="btn-cerrar-modal" data-modal="modal-ventas-metodo" style="background: none; border: none; font-size: 1.5rem; cursor: pointer;">
                        <i class="fas fa-times"></i>
                    </button>
                </div>
                <div style="padding: 20px; max-height: 400px; overflow-y: auto;">
                    <div style="background: #48bb78; color: white; padding: 10px; border-radius: 8px; margin-bottom: 15px;">
                        <strong>Total acumulado:</strong> $${total.toFixed(2)}
                    </div>
        `;
        ...
        ventasFiltradas.forEach(venta => {
            ...
            html += `
                <div style="border: 1px solid #e2e8f0; border-radius: 8px; padding: 12px; margin-bottom: 10px;">
                    <div><strong>#${venta.id}</strong> - ${fecha}</div>
                    <div>Total: $${(venta.total_final || venta.total).toFixed(2)}</div>
                    <div>Productos: ${venta.productos.map(p => `${p.cantidad} x ${p.nombre}`).join(', ')}</div>
                    <button class="btn btn-sm btn-info" onclick="app.mostrarDetalleVenta(${JSON.stringify(venta).replace(/"/g, '&quot;')})">Ver detalle</button>
                </div>
            `;
        });
        ...
        html += `
                ...
                <div class="form-actions" style="padding: 20px;">
                    <button class="btn btn-secondary btn-cerrar-modal" data-modal="modal-ventas-metodo">Cerrar</button>
                </div>
            </div>
        `;

        modal.innerHTML = html;
        this.abrirModal('modal-ventas-metodo');
    }
                ...
                <form id="form-ingreso-caja">
                    ...
                    <div class="form-group">
                        ...
                        <select id="ingreso-metodo" class="form-control" required>
                            ...
                            <option value="mercadopago">Banco Nación</option>
                        </select>
                    </div>
                    <div class="form-group">
    ...
    // ========== MODIFICADO: mostrarSaldoCajaInterno con botones de filtro ==========
        ...
        modal.innerHTML = `
            <div class="modal-content" style="max-width: 600px;">
                ...
                <div style="padding: 20px;">
                        ...
                        ${metodos.map(metodo => {
                            ...
                            return `
                                <div style="display: flex; justify-content: space-between; padding: 12px 15px; border-bottom: 1px solid #e2e8f0;">
                                    <span style="font-weight: 600; text-transform: capitalize;">${metodo === 'mercadopago' ? 'Banco Nación' : metodo}</span>
                    ...
                    <!-- ========== NUEVO: Botones de filtro por método de pago ========== -->
                    <div style="margin: 20px 0 15px; display: flex; gap: 10px; flex-wrap: wrap; justify-content: center; border-top: 2px solid #e2e8f0; padding-top: 20px;">
                        <button class="btn btn-outline" onclick="app.mostrarVentasPorMetodo('efectivo')" style="border: 1px solid #4a5568; background: white; padding: 8px 16px; border-radius: 6px; cursor: pointer;">
                            <i class="fas fa-money-bill-wave"></i> Efectivo
                        </button>
                        <button class="btn btn-outline" onclick="app.mostrarVentasPorMetodo('transferencia')" style="border: 1px solid #4a5568; background: white; padding: 8px 16px; border-radius: 6px; cursor: pointer;">
                            <i class="fas fa-university"></i> Transferencia
                        </button>
                        <button class="btn btn-outline" onclick="app.mostrarVentasPorMetodo('debito')" style="border: 1px solid #4a5568; background: white; padding: 8px 16px; border-radius: 6px; cursor: pointer;">
                            <i class="fas fa-credit-card"></i> Débito
                        </button>
                        <button class="btn btn-warning" onclick="app.mostrarVentasPendientes()" style="border: none; background: #ed8936; color: white; padding: 8px 16px; border-radius: 6px; cursor: pointer;">
                            <i class="fas fa-clock"></i> Crédito (Pendientes)
                        </button>
                    </div>
                    <!-- ============================================================== -->
                                ...
                                <div style="border: 1px solid #e2e8f0; border-radius: 8px; padding: 12px; margin-bottom: 8px; position: relative; background: #f9f9f9;">
                                    ...
                                    <div style="display: flex; justify-content: space-between; align-items: center;">
                                        <div>
                                            ...
                                            <span style="margin-left: 10px; font-size: 0.9rem; color: #718096;">${mov.metodo === 'mercadopago' ? 'Banco Nación' : mov.metodo}</span>
    ...
    mostrarDetalleMovimiento(movimiento) {
        ...
        if (tipo === 'ingreso' && referencia) {
            ...
            alert(`Movimiento: ${concepto}\nMonto: $${monto.toFixed(2)}\nMétodo: ${metodo === 'mercadopago' ? 'Banco Nación' : metodo}\nFecha: ${new Date(fecha).toLocaleString()}`);
        }
    }
        ...
        let html = `
            <div class="modal-content" style="max-width: 600px;">
                ...
                <div style="padding: 20px;">
                    ...
                    ${venta.pagos ? `<div><strong>Pagos mixtos:</strong> ${venta.pagos.map(p => `${p.metodo}: $${p.monto.toFixed(2)}`).join(', ')}</div>` : ''}
            ...
            html += `
                <div style="border: 1px solid #e2e8f0; border-radius: 12px; padding: 20px; margin-bottom: 25px; background: white;">
                    ...
                    <div style="display: grid; grid-template-columns: repeat(2,1fr); gap: 10px; margin-bottom: 15px;">
                        ...
                        ${cierre.cantidadPendientes !== undefined ? `<div><strong>Pendientes:</strong> ${cierre.cantidadPendientes} ($${cierre.totalPendiente.toFixed(2)})</div>` : ''}
                    </div>
    ...
    // ========== FUNCIÓN ACTUALIZADA: generarResumenCierreCaja (sin botones de filtro) ==========
    generarResumenCierreCaja() {
        const ventasDelDia = JSON.parse(localStorage.getItem('ventasDelDia')) || [];
        const gastosDelDia = JSON.parse(localStorage.getItem('gastosDelDia')) || [];
        const caja = JSON.parse(localStorage.getItem('caja')) || { saldo: 0 };
        
        // 👇 Obtener ventas pendientes y calcular total pendiente neto del día
        const ventasPendientes = this.cargarVentasPendientes();
        const hoy = new Date().toISOString().split('T')[0];
        const pendientesHoy = ventasPendientes.filter(v => v.fecha && v.fecha.startsWith(hoy));
        const totalPendienteNeto = pendientesHoy.reduce((sum, v) => sum + (v.total_neto || 0), 0);
        ...
        ventasDelDia.forEach(venta => {
            const totalBaseVenta = venta.total_final !== undefined ? venta.total_final : venta.total;
            totalVentas += totalBaseVenta;

            // Si es pago mixto (tiene array 'pagos' y 'comisiones_detalle')
            if (venta.pagos && venta.comisiones_detalle) {
                venta.comisiones_detalle.forEach(det => {
                    ...
                    ventasPorMetodo[metodo].total += det.monto;
                    ventasPorMetodo[metodo].neto += det.neto;
                    ventasPorMetodo[metodo].comisiones += det.comision_monto;
                    ventasPorMetodo[metodo].cantidad += 1;
                    totalComisiones += det.comision_monto;
                    totalNeto += det.neto;
                });
            } else {
                // Venta simple (un solo método)
                const metodo = venta.metodo_pago;
                const comisionPorcentaje = this.comisiones[metodo] || 0;
                const comisionMonto = totalBaseVenta * (comisionPorcentaje / 100);
                const neto = totalBaseVenta - comisionMonto;
                if (!ventasPorMetodo[metodo]) {
                    ventasPorMetodo[metodo] = {
                        total: 0,
                        neto: 0,
                ...
                ventasPorMetodo[metodo].total += totalBaseVenta;
                ventasPorMetodo[metodo].neto += neto;
                ventasPorMetodo[metodo].comisiones += comisionMonto;
                ventasPorMetodo[metodo].cantidad += 1;
        ...
        const totalGastos = gastosDelDia.reduce((sum, gasto) => sum + gasto.monto, 0);
        // 👇 Restar el total pendiente neto al total final
        const totalFinal = totalNeto - totalGastos - totalPendienteNeto;
        
        // Calcular saldo incluyendo pendientes (para mostrar)
        const saldoConPendientes = caja.saldo + totalPendienteNeto;
        
        // Construcción del HTML (sin botones de filtro)
        let html = `
            <div style="margin-bottom: 20px;">
                ...
                <div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; padding: 20px; border-radius: 10px; margin-bottom: 25px;">
                    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px;">
                        <span style="font-size: 1.1rem;">Saldo en caja actual:</span>
                        <span style="font-size: 1.5rem; font-weight: 700;">$${caja.saldo.toFixed(2)}</span>
                    </div>
                    ${totalPendienteNeto > 0 ? `
                    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px;">
                        <span style="font-size: 1.1rem;">Ventas pendientes (neto):</span>
                        <span style="font-size: 1.3rem; font-weight: 600; color: #fbd38d;">+$${totalPendienteNeto.toFixed(2)}</span>
                    </div>
                    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px; border-top: 1px dashed rgba(255,255,255,0.3); padding-top: 10px;">
                        <span style="font-size: 1.1rem; font-weight: 600;">SALDO + PENDIENTES:</span>
                        <span style="font-size: 1.8rem; font-weight: 700; color: #c6f6d5;">$${saldoConPendientes.toFixed(2)}</span>
                    </div>
                    ` : ''}
                    <div style="border-top: 2px solid rgba(255,255,255,0.3); margin: 15px 0 10px; padding-top: 15px;">
                        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px;">
                            <span style="font-size: 1.1rem;">Ventas del día (bruto):</span>
                            <span style="font-size: 1.3rem; font-weight: 600;">$${totalVentas.toFixed(2)}</span>
                        </div>
                        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px;">
                            <span style="font-size: 1.1rem;">Total comisiones:</span>
                            <span style="font-size: 1.2rem; font-weight: 600; color: #fed7d7;">-$${totalComisiones.toFixed(2)}</span>
                        </div>
                        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px;">
                            <span style="font-size: 1.1rem;">Total gastos:</span>
                            <span style="font-size: 1.2rem; font-weight: 600; color: #fed7d7;">-$${totalGastos.toFixed(2)}</span>
                        </div>
                        ${totalPendienteNeto > 0 ? `
                        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px;">
                            <span style="font-size: 1.1rem;">Ventas pendientes (neto):</span>
                            <span style="font-size: 1.2rem; font-weight: 600; color: #fbd38d;">-$${totalPendienteNeto.toFixed(2)}</span>
                        </div>
                        ` : ''}
                        <div style="border-top: 2px solid rgba(255,255,255,0.3); margin-top: 10px; padding-top: 10px;">
                            <div style="display: flex; justify-content: space-between; align-items: center;">
                ...
                html += `
                    <div style="background: white; border: 1px solid #e2e8f0; border-radius: 8px; padding: 15px; margin-bottom: 10px;">
                        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
                            <div>
                                <strong style="text-transform: capitalize; font-size: 1.1rem;">${metodo === 'mercadopago' ? 'Banco Nación' : metodo}</strong>
        ...
        // Resto del código HTML para gastos y totales
        if (gastosDelDia.length > 0) {
        ...
        // Ya no se incluyen los botones de filtro aquí (están en el modal de saldo)
        html += `
            <div style="margin-top: 25px; padding: 15px; background: #f7fafc; border-radius: 8px; border-left: 4px solid #48bb78;">
                ...
                ${totalPendienteNeto > 0 ? `
                <div style="display: flex; justify-content: space-between; margin-bottom: 5px;">
                    <span>Pendientes (neto):</span>
                    <span style="font-weight: 600; color: #ed8936;">-$${totalPendienteNeto.toFixed(2)}</span>
                </div>
                ` : ''}
    ...
    // ========== FUNCIÓN ACTUALIZADA: obtenerResumenVentasPorMetodo ==========
        ...
        ventas.forEach(venta => {
            ...
            if (venta.pagos && venta.comisiones_detalle) {
                venta.comisiones_detalle.forEach(det => {
                    const metodo = det.metodo;
                    if (!resumen[metodo]) {
                        resumen[metodo] = { total: 0, cantidad: 0 };
                    }
                    resumen[metodo].total += det.monto;
                ...
                resumen[metodo].total += totalBaseVenta;
                resumen[metodo].cantidad += 1;
            }
        });
        return resumen;
    ...
    async registrarCierreCaja() {
        if (confirm('¿Registrar cierre de caja y limpiar datos del día?')) {
            ...
            const pendientes = this.cargarVentasPendientes();
            const pendientesHoy = pendientes.filter(v => v.fecha && v.fecha.startsWith(hoy));
            const totalPendienteNeto = pendientesHoy.reduce((sum, v) => sum + (v.total_neto || 0), 0);
            ...
            const cierre = {
                ...
                netoDepositar: totalNeto - totalGastos - totalPendienteNeto,
                ...
                cantidadPendientes: pendientesHoy.length,
                totalPendiente: totalPendienteNeto
            };
                ...
                html += `
                    <div style="border: 1px solid #e2e8f0; border-radius: 8px; padding: 15px; margin-bottom: 10px;">
                        ...
                        <div style="margin-top: 10px;">
                            ...
                            ${cierre.cantidadPendientes !== undefined ? `<div>Pendientes: ${cierre.cantidadPendientes} ($${cierre.totalPendiente.toFixed(2)})</div>` : ''}
            ...
            <div id="botones-pago" style="margin-top: 20px;">
                <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 10px; margin-bottom: 15px;">
                    ...
                    <button class="btn-pago btn-mercadopago ${this.metodoPagoSeleccionado === 'mercadopago' ? 'activo' : ''}" 
                        ...
                        <i class="fas fa-mobile-alt"></i> Banco Nación
                    </button>
                </div>
                <div style="display: flex; gap: 10px;">
                    <button class="btn btn-success" onclick="app.abrirProcesarPago()" 
                            style="flex:1; padding: 12px; background: var(--ingreso); color: white; border: none; border-radius: 8px; font-weight: 600; cursor: pointer;">
                        <i class="fas fa-check"></i> Pago Simple
                    </button>
                    <button class="btn btn-primary" onclick="app.abrirPagoMixto()" 
                            style="flex:1; padding: 12px; background: #9b59b6; color: white; border: none; border-radius: 8px; font-weight: 600; cursor: pointer;">
                        <i class="fas fa-layer-group"></i> Pago Mixto
                    </button>
                </div>
            </div>
        `;
    ...
    // ==================== FUNCIONES DE PAGO SIMPLE (con descuento/recargo - original) ====================
    abrirProcesarPago() {
        if (this.carrito.length === 0) {
        ...
        if (metodo === 'efectivo') {
            ...
            // 👇 NUEVO BLOQUE PARA BANCO NACIÓN
            seccionMetodo = `
                <div id="seccion-banco-nacion" style="margin-top: 20px; padding: 15px; background: #f8fafc; border-radius: 8px;">
                    <h4 style="margin-bottom: 15px; color: var(--dark);">Banco Nación</h4>
                    <div style="display: flex; gap: 15px; margin-bottom: 15px;">
                        <button class="btn btn-outline btn-banco-debito" onclick="app.bancoNacionTipo='debito'; app.actualizarBancoNacionUI();" style="background: ${this.bancoNacionTipo === 'debito' ? '#2b6cb0' : '#e2e8f0'}; color: ${this.bancoNacionTipo === 'debito' ? 'white' : 'black'};">
                            <i class="fas fa-credit-card"></i> Débito
                        </button>
                        <button class="btn btn-outline btn-banco-credito" onclick="app.bancoNacionTipo='credito'; app.actualizarBancoNacionUI();" style="background: ${this.bancoNacionTipo === 'credito' ? '#2b6cb0' : '#e2e8f0'}; color: ${this.bancoNacionTipo === 'credito' ? 'white' : 'black'};">
                            <i class="fas fa-credit-card"></i> Crédito
                        </button>
                    </div>
                    <div id="seccion-recargo-banco" style="display: ${this.bancoNacionTipo === 'credito' ? 'block' : 'none'};">
                        <div class="form-group">
                            <label>Recargo manual (%)</label>
                            <input type="number" id="recargo-banco-manual" class="form-control" min="0" step="0.1" value="0">
                            <small style="color: #718096;">Porcentaje de recargo que se suma al total.</small>
                        </div>
                        <button class="btn btn-primary" id="btn-aplicar-recargo-banco">Aplicar recargo</button>
                    </div>
                    <p style="margin-top: 10px; font-size: 0.9rem; color: #718096;">${this.bancoNacionTipo === 'debito' ? 'Pago inmediato sin recargo.' : 'Pago con recargo, se registrará como pendiente.'}</p>
                </div>
            `;
        ...
        modal.innerHTML = `
            <div class="modal-content">
                <div class="modal-header">
                    <h3><i class="fas fa-credit-card"></i> Procesar Pago (Simple)</h3>
                ...
                <div style="padding: 20px;">
                    <div style="text-align: center; margin-bottom: 30px;">
                        ...
                        <div style="font-size: 0.9rem; color: #718096; margin-top: 5px;">
                            Método: <span id="metodo-pago-seleccionado">${metodo === 'mercadopago' ? 'Banco Nación' : metodo.charAt(0).toUpperCase() + metodo.slice(1)}</span>
                        </div>
        ...
        const actualizarTotalFinal = () => {
            ...
            if (metodo === 'efectivo') {
                ...
                this.calcularVueltoSimple(totalFinal);
            }
        };
        ...
        // ========== BLOQUE PARA BANCO NACIÓN ==========
        if (metodo === 'mercadopago') {
            // Función para aplicar recargo manual
            const btnAplicarRecargoBanco = document.getElementById('btn-aplicar-recargo-banco');
            if (btnAplicarRecargoBanco) {
                btnAplicarRecargoBanco.addEventListener('click', () => {
                    if (this.bancoNacionTipo === 'credito') {
                        const recargo = parseFloat(document.getElementById('recargo-banco-manual').value) || 0;
                        if (recargo > 0) {
                            totalFinal = totalBase * (1 + recargo / 100);
                            actualizarTotalFinal();
                        } else {
                            totalFinal = totalBase;
                            actualizarTotalFinal();
            ...
            // Definir función para actualizar UI de Banco Nación
            this.actualizarBancoNacionUI = function() {
                const btns = document.querySelectorAll('.btn-banco-debito, .btn-banco-credito');
                btns.forEach(btn => {
                    btn.style.background = '#e2e8f0';
                    btn.style.color = 'black';
                });
                const btnActivo = document.querySelector(`.btn-banco-${this.bancoNacionTipo}`);
                if (btnActivo) {
                    btnActivo.style.background = '#2b6cb0';
                    btnActivo.style.color = 'white';
                }
                const seccionRecargo = document.getElementById('seccion-recargo-banco');
                if (seccionRecargo) {
                    seccionRecargo.style.display = this.bancoNacionTipo === 'credito' ? 'block' : 'none';
                }
                // Actualizar total según tipo
                if (this.bancoNacionTipo === 'debito') {
                    totalFinal = totalBase;
                    actualizarTotalFinal();
                } else {
                    const recargo = parseFloat(document.getElementById('recargo-banco-manual')?.value) || 0;
                ...
                const infoP = document.querySelector('#seccion-banco-nacion p');
                if (infoP) {
                    infoP.textContent = this.bancoNacionTipo === 'debito' ? 'Pago inmediato sin recargo.' : 'Pago con recargo, se registrará como pendiente.';
                }
            };
            
            // Inicializar la UI
            setTimeout(() => {
                if (typeof this.actualizarBancoNacionUI === 'function') {
                    this.actualizarBancoNacionUI();
                }
            }, 100);
        }
        // ================================================
        ...
        const montoRecibido = document.getElementById('monto-recibido');
        if (montoRecibido) {
            montoRecibido.addEventListener('input', () => this.calcularVueltoSimple(totalFinal));
            montoRecibido.focus();
            montoRecibido.select();
        }
        
        this.calcularVueltoSimple = (total) => {
        ...
        // Asignar evento al botón finalizar (modificado para pasar esCreditoBanco)
        document.getElementById('btn-finalizar-venta').onclick = () => {
            let metodoFinal = metodo;
            let esCreditoBanco = false;
            if (metodo === 'mercadopago') {
                if (this.bancoNacionTipo === 'credito') {
                    esCreditoBanco = true;
                    metodoFinal = 'banco_nacion_credito';
                } else {
                    metodoFinal = 'banco_nacion_debito';
                }
            }
            this.finalizarVentaSimple(totalFinal, totalBase, metodoFinal, descuentoAplicado, recargoAplicado, esCreditoBanco);
        };
    }
    
    // ========== MODIFICADO: finalizarVentaSimple con parámetro esCreditoBanco ==========
    finalizarVentaSimple(totalFinal, totalBase, metodo, descuentoAplicado, recargoAplicado, esCreditoBanco = false) {
        if (this.carrito.length === 0) {
            this.mostrarError('No hay productos en el carrito');
            return;
        }
        ...
        if (metodo === 'efectivo') {
            ...
            vuelto = montoRecibido - totalFinal;
        }
        ...
        // 👇 Si es crédito de Banco Nación, guardar como pendiente y salir
        if (esCreditoBanco || metodo === 'banco_nacion_credito') {
            const pendientes = this.cargarVentasPendientes();
            pendientes.push(venta);
            this.guardarVentasPendientes(pendientes);
            this.guardarPendienteEnBD(venta);
            this.actualizarStockProductos('restar', this.carrito);
            this.carrito = [];
            this.actualizarCarrito();
            this.cerrarModal('modal-procesar-pago');
            this.mostrarNotificacion(`⏳ Venta #${venta.id} registrada como PENDIENTE (Crédito Banco Nación). Total: $${totalFinal.toFixed(2)} (se cobrará manualmente).`);
            return;
        }
    ...
    abrirPagoMixto() {
        ...
        let modal = document.getElementById('modal-pago-mixto');
        if (!modal) {
            modal = document.createElement('div');
            modal.id = 'modal-pago-mixto';
            modal.className = 'modal';
            document.body.appendChild(modal);
        }
        
        let pagos = []; // array de {metodo, monto}
        
        const actualizarResumen = () => {
            const totalPagado = pagos.reduce((sum, p) => sum + p.monto, 0);
            const faltante = totalBase - totalPagado;
            document.getElementById('total-pagado-mixto').textContent = `$${totalPagado.toFixed(2)}`;
            const faltanteSpan = document.getElementById('faltante-mixto');
            faltanteSpan.textContent = `$${Math.max(0, faltante).toFixed(2)}`;
            faltanteSpan.style.color = faltante > 0 ? '#e53e3e' : '#48bb78';
            const btnFinalizar = document.getElementById('btn-finalizar-mixto');
            if (btnFinalizar) btnFinalizar.disabled = (faltante > 0);
            
            const tbody = document.getElementById('tabla-pagos-mixtos');
            if (tbody) {
                if (pagos.length === 0) {
                    ...
                    tbody.innerHTML = pagos.map((p, idx) => `
                        <tr>
                            <td>${p.metodo === 'mercadopago' ? 'Banco Nación' : p.metodo.toUpperCase()}</td>
                            <td>$${p.monto.toFixed(2)}</td>
                            <td><button class="btn-eliminar-pago-mixto" data-index="${idx}" style="background:none;border:none;color:#e53e3e;cursor:pointer;"><i class="fas fa-trash"></i></button></td>
                        </tr>
                    `).join('');
                    document.querySelectorAll('.btn-eliminar-pago-mixto').forEach(btn => {
        ...
        const agregarPago = () => {
            const metodo = document.getElementById('metodo-pago-mixto').value;
            let monto = parseFloat(document.getElementById('monto-pago-mixto').value);
            if (isNaN(monto) || monto <= 0) {
                alert('Ingrese un monto válido');
                return;
            }
            const totalPagadoActual = pagos.reduce((sum, p) => sum + p.monto, 0);
            const faltante = totalBase - totalPagadoActual;
            if (monto > faltante) {
                alert(`El monto no puede superar el faltante ($${faltante.toFixed(2)})`);
                return;
            }
            pagos.push({ metodo, monto });
            document.getElementById('monto-pago-mixto').value = '';
            actualizarResumen();
            document.getElementById('monto-pago-mixto').focus();
        };
        
        modal.innerHTML = `
            <div class="modal-content" style="max-width: 700px;">
                <div class="modal-header">
                    <h3><i class="fas fa-layer-group"></i> Pago Mixto</h3>
                    <button class="btn-cerrar-modal" data-modal="modal-pago-mixto" style="background: none; border: none; font-size: 1.5rem; cursor: pointer;">
                        <i class="fas fa-times"></i>
                    </button>
                </div>
                <div style="padding: 20px;">
                    <div style="text-align:center; margin-bottom:20px;">
                        <div style="font-size:0.9rem;">Total de la compra</div>
                        <div style="font-size:2rem;font-weight:700;">$${totalBase.toFixed(2)}</div>
                    </div>
                    <div style="background:#f8fafc; border-radius:12px; padding:20px; margin-bottom:20px;">
                        <h4>Agregar pago parcial</h4>
                        <div style="display:flex; gap:15px; flex-wrap:wrap;">
                            <select id="metodo-pago-mixto" class="form-control" style="flex:2;">
                                <option value="efectivo">Efectivo</option>
                                <option value="debito">Débito</option>
                                <option value="credito">Crédito</option>
                                <option value="transferencia">Transferencia</option>
                            ...
                            <input type="number" id="monto-pago-mixto" class="form-control" placeholder="Monto" step="0.01" min="0.01" style="flex:2;">
                            <button id="btn-agregar-pago-mixto" class="btn btn-primary" style="flex:1;">Agregar</button>
                        </div>
                    </div>
                    <div>
                        <h4>Pagos registrados</h4>
                        <table style="width:100%; border-collapse: collapse; border:1px solid #e2e8f0;">
                            <thead style="background:#e2e8f0;"><tr><th>Método</th><th>Monto</th><th></th></tr></thead>
                            <tbody id="tabla-pagos-mixtos"><tr><td colspan="3" style="text-align:center;">No hay pagos</td></tr></tbody>
                        </table>
                    </div>
                    <div style="margin-top:20px; padding:15px; background:#e6fffa; border-radius:10px;">
                        <div style="display:flex; justify-content:space-between;"><span>Total pagado:</span><span id="total-pagado-mixto">$0.00</span></div>
                        <div style="display:flex; justify-content:space-between;"><span>Faltante:</span><span id="faltante-mixto">$${totalBase.toFixed(2)}</span></div>
                    </div>
                    <div class="form-actions" style="margin-top:30px;">
                        <button class="btn btn-secondary btn-cerrar-modal" data-modal="modal-pago-mixto">Cancelar</button>
                        <button id="btn-finalizar-mixto" class="btn btn-success" disabled>Finalizar Venta</button>
                    </div>
                </div>
            </div>
        `;
        
        this.abrirModal('modal-pago-mixto');
        
        document.getElementById('btn-agregar-pago-mixto').addEventListener('click', agregarPago);
        document.getElementById('monto-pago-mixto').addEventListener('keypress', (e) => { if(e.key === 'Enter') agregarPago(); });
        document.getElementById('btn-finalizar-mixto').addEventListener('click', () => {
            const totalPagado = pagos.reduce((sum,p)=>sum+p.monto,0);
            if(totalPagado < totalBase) {
                alert('No se ha cubierto el total de la compra');
                return;
            }
            const excedente = totalPagado - totalBase;
            if(excedente > 0) {
                const pagoEfectivo = pagos.find(p => p.metodo === 'efectivo');
                if(!pagoEfectivo || pagoEfectivo.monto < excedente) {
                    alert('El excedente solo puede ser pagado en efectivo para dar vuelto');
                    return;
                }
            }
            this.finalizarVentaMixta(pagos, totalBase);
        });
    }
    
    // ========== MODIFICADO: finalizarVentaMixta con guardado de pendientes ==========
    finalizarVentaMixta(pagos, totalBase) {
        const totalPagado = pagos.reduce((sum,p)=>sum+p.monto,0);
        const excedente = totalPagado - totalBase;
        let vueltoEfectivo = 0;
        if(excedente > 0) vueltoEfectivo = excedente;
        
        const comisionesDetalle = pagos.map(pago => {
            const porcentaje = this.comisiones[pago.metodo] || 0;
            const comision = pago.monto * (porcentaje / 100);
            return {
                metodo: pago.metodo,
                monto: pago.monto,
                comision_porcentaje: porcentaje,
                comision_monto: comision,
                neto: pago.monto - comision
            };
        });
        ...
        const venta = {
            ...
            total_comisiones: comisionesDetalle.reduce((sum,c)=>sum+c.comision_monto,0),
            total_neto: comisionesDetalle.reduce((sum,c)=>sum+c.neto,0)
        };
        
        // 👇 Si incluye crédito (o mercadopago), guardar como pendiente
        const tieneCredito = pagos.some(p => p.metodo === 'credito' || p.metodo === 'mercadopago');
        if (tieneCredito) {
            const pendientes = this.cargarVentasPendientes();
            pendientes.push(venta);
            this.guardarVentasPendientes(pendientes);
            ...
            this.carrito = [];
            this.actualizarCarrito();
            this.cerrarModal('modal-pago-mixto');
            this.mostrarNotificacion(`⏳ Venta mixta #${venta.id} registrada como PENDIENTE (incluye crédito). Total: $${totalBase.toFixed(2)}`);
            return;
        }
        ...
        venta.comisiones_detalle.forEach(det => {
            this.agregarIngreso(det.neto, `Venta #${venta.id} (${det.metodo})`, det.metodo, venta.id.toString());
        });
        if(vueltoEfectivo > 0) {
            this.agregarEgreso(vueltoEfectivo, `Vuelto de venta #${venta.id}`, 'efectivo', venta.id.toString());
        }
        
        this.carrito = [];
        this.actualizarCarrito();
        this.cerrarModal('modal-pago-mixto');
        this.mostrarNotificacion(`✅ Venta mixta #${venta.id} registrada. Total: $${totalBase.toFixed(2)} (Neto: $${venta.total_neto.toFixed(2)})`);
    ...
    // ==================== IMPRESIÓN DE TICKET (actualizada para pagos mixtos) ====================
    imprimirTicket(venta) {
        ...
        html += `
                    ...
                    <div class="line"></div>
                    <div class="total-line"><strong>TOTAL:</strong><strong>$${venta.total.toFixed(2)}</strong></div>
        `;

        if (venta.pagos && venta.pagos.length > 0) {
            html += `<div class="line"></div><div><strong>Pagos:</strong></div>`;
            venta.pagos.forEach(pago => {
                const comisionPorc = this.comisiones[pago.metodo] || 0;
                html += `<div class="total-line"><span>${pago.metodo === 'mercadopago' ? 'Banco Nación' : pago.metodo.toUpperCase()}:</span><span>$${pago.monto.toFixed(2)}</span></div>`;
                if (comisionPorc > 0) {
                    html += `<div class="total-line" style="font-size:10px;"><span>  Comisión (${comisionPorc}%):</span><span>-$${(pago.monto * comisionPorc / 100).toFixed(2)}</span></div>`;
                }
            });
            html += `<div class="total-line"><strong>Neto recibido:</strong><strong>$${venta.total_neto.toFixed(2)}</strong></div>`;
        } else {
            if (venta.comision_monto) {
